...
/**
 * 分页用表格数据集(java.util.Map)构造器, 实现GridDatasetProducer接口的抽象类
 * 
 * @author 韦海晗
 */
public class AbstractGridDatasetProducer implements GridDatasetProducer {
	/**
	...
	public Map produceDataset(Integer start, Integer limit, String sort, String direction, String root, String totalProperty, String successProperty, Map params) throws Exception {
        ...
        if(successProperty != null){
        	dataset.put(successProperty, isSuccess());
        }
        if(totalProperty != null){
        	dataset.put(totalProperty, getTotalRecords(params));
        }
        if(root != null){
        	dataset.put(root, getRecords(start, limit, sort, direction, params));
        }
		return dataset;
	}
